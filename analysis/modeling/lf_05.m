...
output = [out_path cell_type '_05/' cell_type '_05_' datestr(now,'yyyy-mm-dd-HH-MM-SS') '.mat'];
...
for metabolite = {'AKG' 'GLU' 'CIT' 'ASP' 'MAL' 'ALA'}
    m.expts{'[U-13C6]GLC'}.data_ms{char(metabolite)}.mdvs.on = [true true false];
end

% remove low labeled 3-carbon metabolites
for metabolite = {'LAC' 'PYR'}
    m.expts{'[1,2-13C2]GLC'}.data_ms{char(metabolite)}.on = [false];
end

% remove first timepoint for these glc6 metabolites
for metabolite = {'PYR' 'LAC'}
    m.expts{'[U-13C6]GLC'}.data_ms{char(metabolite)}.mdvs.on = [false true true];
end
